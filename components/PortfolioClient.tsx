...
declare global {
  interface Window {
    ...
    playThemeChime?: (isToLight: boolean) => void;
  }
}
...
export default function PortfolioClient() {
  useEffect(() => {
    ...
    // Helper to wait for external script objects if needed
    function waitForLibs(callback: () => void, maxTries = 30) {
      if (
        ...
        setTimeout(() => waitForLibs(callback, maxTries - 1), 100);
      } else {
        callback();
      }
    }
    ...
    waitForLibs(() => {
      ...
      function nextStep() {
        ...
        setTimeout(
          nextStep,
          si === steps.length ? 250 : Math.random() * 180 + 120
        );
      }

      setTimeout(nextStep, 150);

      const handleEscapeSplash = (e: KeyboardEvent) => {
      ...
      document.addEventListener("keydown", handleEscapeSplash);
      ...
      window.showNotif = function (msg: string, type = "success") {
      ...
      const certFilterBtns = document.querySelectorAll(
        ".cert-filter-btn"
      ...
      certCards.forEach((card) => {
        ...
        if (imgWrap) {
          imgWrap.addEventListener("click", (e) => {
            e.stopPropagation();
            const img = imgWrap.querySelector("img") as HTMLImageElement;
      ...
      if (certLightbox) {
        certLightbox.addEventListener("click", (e) => {
          if (
            ...
            (e.target as HTMLElement).classList.contains("lightbox-close")
          ) {
            closeLightboxFunc();
          }
        });
      ...
      function startAnimations() {
        ...
        document
          .querySelectorAll(".fade-in, .fade-in-l, .fade-in-r")
        ...
        document
          .querySelectorAll(".skills-progress")
          .forEach((el) => pobs.observe(el));
        ...
        const cobs = new IntersectionObserver(
          (entries) => {
            entries.forEach((e) => {
              if (e.isIntersecting) {
                e.target.querySelectorAll(".countup").forEach((el: any) => {
      ...
      const tiltCards = document.querySelectorAll(
        ".bento-card, .skill-card"
      ) as NodeListOf<HTMLElement>;
      tiltCards.forEach((card) => {
        const inner =
          (card.querySelector(".card-inner") as HTMLElement) || card;
      ...
      const heroImageSide = document.querySelector(
        ".hero-image-side"
      ) as HTMLElement;
      const mainFrame = document.querySelector(
        ".hero-img-frame"
      ) as HTMLElement;
      const badgeTop = document.querySelector(".badge-top") as HTMLElement;
      const badgeBottom = document.querySelector(
        ".badge-bottom"
      ) as HTMLElement;
      const ft1 = document.querySelector(".ft-1") as HTMLElement;
      const ft2 = document.querySelector(".ft-2") as HTMLElement;
      const ft3 = document.querySelector(".ft-3") as HTMLElement;
      ...
      if (heroSection && heroImageSide) {
        heroSection.addEventListener(
          ...
          (e) => {
            ...
            heroImageSide.style.transform = `perspective(1200px) rotateX(${
              -y * 8
            }deg) rotateY(${x * 8}deg)`;
            if (mainFrame)
              mainFrame.style.transform = `translateZ(20px) translate3d(${
                x * 8
              }px, ${y * 8}px, 0)`;
            if (badgeTop)
              badgeTop.style.transform = `translateZ(50px) translate3d(${
                x * 20
              }px, ${y * 20}px, 0) translateX(10px) translateY(-5px)`;
            if (badgeBottom)
              badgeBottom.style.transform = `translateZ(40px) translate3d(${
                x * 16
              }px, ${y * 16}px, 0) translateX(-10px) translateY(5px)`;
            if (ft1)
              ft1.style.transform = `translateZ(60px) translate3d(${
                x * -25
              }px, ${y * -25}px, 0) rotate(-10deg)`;
            if (ft2)
              ft2.style.transform = `translateZ(80px) translate3d(${
                x * -32
              }px, ${y * -32}px, 0) rotate(15deg)`;
            if (ft3)
              ft3.style.transform = `translateZ(50px) translate3d(${
                x * -20
              }px, ${y * -20}px, 0) rotate(5deg)`;
          },
          { passive: true }
        );

        heroSection.addEventListener("mouseleave", () => {
          heroImageSide.style.transform =
            "perspective(1200px) rotateX(0deg) rotateY(0deg)";
          if (mainFrame)
            mainFrame.style.transform =
              "translateZ(20px) translate3d(0, 0, 0)";
          if (badgeTop)
            badgeTop.style.transform =
              "translateZ(50px) translate3d(0, 0, 0) translateX(10px) translateY(-5px)";
          if (badgeBottom)
            badgeBottom.style.transform =
              "translateZ(40px) translate3d(0, 0, 0) translateX(-10px) translateY(5px)";
          if (ft1)
            ft1.style.transform =
              "translateZ(60px) translate3d(0, 0, 0) rotate(-10deg)";
          if (ft2)
            ft2.style.transform =
              "translateZ(80px) translate3d(0, 0, 0) rotate(15deg)";
          if (ft3)
            ft3.style.transform =
              "translateZ(50px) translate3d(0, 0, 0) rotate(5deg)";
        });
      }
      ...
      function initGSAPScrollAnimations() {
        ...
        scrollCards.forEach((card: any, idx: number) => {
          const isAlt = idx % 2 === 0;
          gsap.fromTo(
            card,
            {
      ...
      function getLangColor(lang: string) {
        return (
          ...
          "#" +
            Math.abs(
              lang.split("").reduce((a, c) => a * 31 + c.charCodeAt(0), 0)
            )
              .toString(16)
              .slice(-6)
              .padStart(6, "a")
        ...
        try {
          ...
          if (barsWrap) {
            ...
            repos.forEach((r: any) => {
              if (r.pushed_at) {
                const diff =
                  (now.getTime() - new Date(r.pushed_at).getTime()) /
                  (1000 * 60 * 60 * 24 * 30);
                const idx = Math.floor(diff);
                if (idx >= 0 && idx < 24) monthCounts[idx]++;
              }
            });
            const bars = monthCounts.reverse().map((v) => {
          ...
          if (langList && sortedLangs.length > 0) {
            langList.innerHTML = sortedLangs
              .map(([lang, count]) => {
            ...
            setTimeout(() => {
              langList
                .querySelectorAll(".gh-lang-bar-fill")
                .forEach((el: any) => {
            ...
            langList.innerHTML =
              '<div class="gh-error"><i class="fas fa-code-branch"></i>Belum ada repo publik</div>';
          }

          if (langBarTotal && sortedLangs.length > 0) {
            langBarTotal.innerHTML = sortedLangs
              .map(([lang, count]) => {
                const pct = ((count / totalCount) * 100).toFixed(1);
                return `<div class="gh-lang-bar-segment" style="width:${pct}%;background:${getLangColor(
                  lang
                )}"></div>`;
              })
              .join("");
          ...
          ["gh-loading", "gh-langs-loading"].forEach((id) => {
            ...
            if (el) {
              el.innerHTML =
                '<div class="gh-error"><i class="fas fa-wifi"></i>Tidak dapat memuat data GitHub</div>';
            }
          });
        }
      }
      ...
      function openModal(idx: number) {
        ...
        if (fl) fl.innerHTML = d.features.map((f) => `<li>${f}</li>`).join("");
        if (tl)
          tl.innerHTML = d.tech
            .map((t) => `<span class="tech-pill">${t}</span>`)
            .join("");
        if (liveLink) liveLink.href = d.live;
        if (ghLink) ghLink.href = d.github;
      ...
      document
        .querySelectorAll(".project-card[data-modal-id]")
        .forEach((card: any) => {
      ...
      /* ── DEVELOPER TERMINAL INTERACTIVE SHELL ───────────────── */
      ...
      const commands: Record<string, () => { cls: string; text: string }[]> = {
        help: () => [
          { cls: "accent", text: "┌─────────────────────────────────────┐" },
        ...
        about: () => [
          ...
          {
            cls: "output",
            text: "  Jurusan: Rekayasa Perangkat Lunak | 2024-2027",
          },
        ...
        skills: () => [
          ...
          {
            cls: "success",
            text: "  Frontend  → HTML · CSS · JavaScript · TypeScript · React · Next.js · Tailwind",
          },
          { cls: "success", text: "  Mobile    → Flutter · Dart" },
          {
            cls: "success",
            text: "  Backend   → MySQL · Supabase · REST API · Vercel",
          },
          {
            cls: "success",
            text: "  Tools     → Git · Postman · Figma · Framer Motion · QR Scanner",
          },
          {
            cls: "success",
            text: "  UI/UX     → Wireframing · Prototyping · User Flow · Whimsical",
          },
          { cls: "muted", text: "" },
        ],
        projects: () => [
          ...
          {
            cls: "output",
            text: "  [0] EazyChise (Web Marketplace)     — Next.js · Supabase · Vercel",
          },
          {
            cls: "output",
            text: "  [1] Monsef (Mobile Keuangan)        — Flutter · OCR · Speech-to-Text",
          },
          {
            cls: "output",
            text: "  [2] HaloAir (PDAM Mobile App)       — Flutter · REST API",
          },
          {
            cls: "output",
            text: "  [3] PortaTrip (UI/UX Design)        — Figma · Whimsical",
          },
          {
            cls: "output",
            text: "  [4] Web Visit SMK Telkom            — HTML · CSS · JS",
          },
          {
            cls: "output",
            text: "  [5] Mobile Eazychise                — Flutter · Dart",
          },
          {
            cls: "output",
            text: "  [6] Aplikasi Toko Online            — Fullstack",
          },
          {
            cls: "output",
            text: "  [7] Aplikasi EcoGuard               — Tech Stack",
          },
          {
            cls: "output",
            text: "  [8] Website Mokleters               — Web Dev",
          },
        ...
        hire: () => [
          ...
          {
            cls: "output",
            text: "  Terbuka untuk: Freelance · Internship · Part-time",
          },
          {
            cls: "output",
            text: "  Kirim pesan via form kontak atau email langsung!",
          },
          { cls: "muted", text: "" },
        ],
        theme: () => {
          ...
          return [
            {
              cls: "success",
              text: `  Tema diubah ke: ${isLight ? "☀ Light" : "🌙 Dark"} Mode`,
            },
            { cls: "muted", text: "" },
          ];
        },
      ...
      function printTerminal(lines: { cls: string; text: string }[]) {
        if (!body) return;
        lines.forEach((l) => {
      ...
      function printWelcome() {
        printTerminal([
          ...
          {
            cls: "muted",
            text: '  Ketik "help" untuk melihat perintah yang tersedia.',
          },
          { cls: "muted", text: "" },
        ]);
      }
      ...
      function handleCommand(cmd: string) {
        ...
        history.unshift(cmd);
        histIdx = -1;
        printTerminal([{ cls: "cmd", text: "$ " + cmd }]);
        ...
        if (/^project [0-8]$/.test(lowerCmd)) {
          ...
          printTerminal([
            { cls: "success", text: "  Membuka detail proyek..." },
            { cls: "muted", text: "" },
          ]);
        ...
        if (commands[lowerCmd]) {
          printTerminal(commands[lowerCmd]());
          if (window.playTone) window.playTone(880, "sine", 0.03, 0.04);
        } else {
          printTerminal([
          ...
          if (window.playTone) window.playTone(220, "triangle", 0.03, 0.08);
        }
      }
      ...
      function toggleTerminal() {
        ...
        if (isOpen) {
          ...
          if (body && !body.children.length) printWelcome();
      ...
      document.addEventListener("click", (e) => {
        if (
          ...
          (!navToggle || !navToggle.contains(e.target as Node)) &&
      ...
      if (typeof window.emailjs !== "undefined") {
        window.emailjs.init("F0dnYTmCI7xr50VI6");
      }
      ...
      function showFormStatus(msg: string, color: string) {
      ...
      if (form && sendBtn) {
        form.addEventListener("submit", async (e) => {
          ...
          const subject = (
            document.getElementById("f-subject") as HTMLInputElement
          )?.value.trim();
          const message = (
          ...
          if (!name || !email || !subject || !message) {
            showFormStatus("⚠ Semua field harus diisi.", "#f59e0b");
            return;
          }
          ...
          try {
            ...
            if (btnText) btnText.textContent = "Gagal";
      ...
      /* ── WEB AUDIO API SYNTH FEEDBACK ────────────────────────── */
      let audioCtx: AudioContext | null = null;
      let isMuted = localStorage.getItem("audio_muted") !== "false";
      ...
      if (audioToggle) {
        ...
        audioToggle.addEventListener("click", () => {
          ...
          if (!isMuted) {
            ...
            audioToggle.classList.add("active");
            initAudioContext();
            playTone(523.25, "sine", 0.04, 0.08);
          } else {
            if (audioIcon) audioIcon.className = "fas fa-volume-mute";
            audioToggle.classList.remove("active");
          }
      ...
      function playHoverSound() {
        playTone(1000, "sine", 0.006, 0.02);
      }

      function playClickSound() {
        playTone(700, "sine", 0.04, 0.06);
      }

      function playThemeChime(isToLight: boolean) {
        if (isMuted) return;
        try {
          if (isToLight) {
      ...
      const clickables = document.querySelectorAll(
        "a, button, .project-card, .skill-card, .cert-filter-btn"
      );
